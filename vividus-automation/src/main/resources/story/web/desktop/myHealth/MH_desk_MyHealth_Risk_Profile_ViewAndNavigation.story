...
GivenStories: story/web/desktop/myHealth/MH_desk_MyHealth_LogIn3.story

Scenario: [Desktop][MyHealth: Risk Profile] View and Navigation
...
When I wait `PT20S` with `PT05S` polling until element located `${xpathFor_DownloadMyHealthReportinEnglishPDF}` becomes VISIBLE
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_DownloadMyHealthReportinEnglishHTML}` becomes NOT_VISIBLE
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_Iicon}` becomes VISIBLE
...
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_HighRiskInformationPopupCrossIcon}` becomes VISIBLE
When I click on element located `${xpathFor_HighRiskInformationPopupCrossIcon}`
