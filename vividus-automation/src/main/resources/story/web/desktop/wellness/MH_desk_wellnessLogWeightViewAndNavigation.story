...
When I wait `PT20S` with `PT05S` polling until element located `${xpathFor_fitnessActivity}` becomes VISIBLE
...
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_paginationFWIconInDesk}` becomes VISIBLE
When I click on element located `${xpathFor_paginationFWIconInDesk}`
