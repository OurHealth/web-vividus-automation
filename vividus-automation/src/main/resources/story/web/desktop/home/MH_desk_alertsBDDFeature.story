...
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_alert1_OverTheCounterMedication}` becomes VISIBLE
When I click on element located `${xpathFor_alert1_OverTheCounterMedication}`
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_alert1Title_OverTheCounterMedication}` becomes VISIBLE
When I scroll element located `${xpathFor_alert1_PrivacyPolicyLink}` into view
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_alert1_PrivacyPolicyLink}` becomes VISIBLE
When I click on element located `${xpathFor_alert1_PrivacyPolicyLink}`
When I switch to a new window
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_alert1_PrivacyAndSecurityPolicyInNewWindow}` becomes VISIBLE
