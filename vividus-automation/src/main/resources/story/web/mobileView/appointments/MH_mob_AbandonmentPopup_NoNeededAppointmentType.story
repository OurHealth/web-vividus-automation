...
Scenario: [Mobile][Abandonment popup] No needed appointment type
...
When I wait `PT40S` with `PT10S` polling until element located `${xpathFor_ChooseATimeAndProvider}` becomes VISIBLE
When I wait `PT30S` with `PT10S` polling until element located `${xpathFor_TimeSlotForProvider1}` becomes VISIBLE
When I click on element located `${xpathFor_TimeSlotForProvider1}`
When I wait `PT30S` with `PT10S` polling until element located `${xpathFor_PleaseReviewConfirm}` becomes VISIBLE
When I wait `PT30S` with `PT10S` polling until element located `${xpathFor_profileButton}` becomes VISIBLE
When I click on element located `${xpathFor_profileButton}`
When I wait `PT30S` with `PT10S` polling until element located `${xpathFor_MessagesInProfileMenuInMob}` becomes VISIBLE
When I click on element located `${xpathFor_MessagesInProfileMenuInMob}`
...
When I wait `PT15S` with `PT05S` polling until element located `${xpathFor_ICouldNotFidDateOrTime}` becomes VISIBLE
When I click on element located `${xpathFor_ICouldNotFidDateOrTime}`
...
When I wait `PT15S` with `PT05S` polling until element located `${xpathFor_MessagesHeader}` becomes VISIBLE
...
{
   "reason": "cant-find-date-time"
}
` by JSON path `$.reason` is equal to `cant-find-date-time`
