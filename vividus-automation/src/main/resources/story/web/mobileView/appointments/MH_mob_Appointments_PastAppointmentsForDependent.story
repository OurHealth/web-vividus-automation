...
Scenario: [Mobile][Appointments] Past appointments for dependent
...
When I wait `PT20S` with `PT05S` polling until element located `${xpathFor_mobMenuAppointments}` becomes VISIBLE
When I click on element located `${xpathFor_mobMenuAppointments}`
When I wait `PT20S` with `PT05S` polling until element located `${xpathFor_cmdScheduleAppointmentInMob}` becomes VISIBLE
When I click on element located `${xpathFor_cmdScheduleAppointmentInMob}`
