...
Scenario: [Desktop][Health Summary: Immunizations] View and Navigation
...
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_listOfImmunizationInDesktopView}` becomes VISIBLE
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_immunizationInDesktopView}` becomes VISIBLE
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_immunizationDescription}` becomes VISIBLE
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_AdministeredDate}` becomes VISIBLE
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_AdministeredDate}` becomes VISIBLE
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_AdministeredSite}` becomes VISIBLE
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_sortByImmunization}` becomes VISIBLE
When I click on element located `${xpathFor_sortByImmunization}`
Then elements located `${xpathFor_listOfimmunizationAfterSortInDesktopView}` are sorted by text in CASE_INSENSITIVE_DESCENDING order
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_searchField}` becomes VISIBLE
...
When I wait until element located `${xpathFor_listOfimmunizationAfterSortInDesktopView}` contains text 'COVID'
When I wait `PT10S` with `PT05S` polling until element located `${xpathFor_searchField}` becomes VISIBLE
When I clear field located `${xpathFor_searchField}` using keyboard
