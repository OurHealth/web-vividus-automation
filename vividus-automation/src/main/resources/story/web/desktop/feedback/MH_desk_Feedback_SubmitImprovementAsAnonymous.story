...
Scenario: [Desktop][Feedback] Submit improvement as anonymous
...
When I wait `PT20S` with `PT05S` polling until element located `${xpathFor_SuggestionForImprovement}` becomes VISIBLE
When I click on element located `${xpathFor_SuggestionForImprovement}`
...
When I wait `PT20S` with `PT05S` polling until element located `${xpathFor_SubmitAnonymouslyOption}` becomes VISIBLE
...
When I click on element located `${xpathFor_SubmitAnonymouslyOption}`
When I wait `PT20S` with `PT05S` polling until element located `${xpathFor_AnonymousInNameField}` becomes VISIBLE
...
` by JSON path `$.feedback_type` is equal to `enhancement`
