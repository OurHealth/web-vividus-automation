...
Scenario: [Desktop][Abandonment popup] No proper time/date
...
When I wait `PT15S` with `PT05S` polling until element located `${xpathFor_userProfileButton}` becomes VISIBLE
When I click on element located `${xpathFor_userProfileButton}`
When I wait `PT15S` with `PT05S` polling until element located `${xpathFor_SignOutButtonInProfile}` becomes VISIBLE
...
When I wait `PT15S` with `PT05S` polling until element located `${xpathFor_signInHeader}` becomes VISIBLE
